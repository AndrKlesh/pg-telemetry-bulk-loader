...
pub struct Arguments {
    // public - структура будет доступна для всех модулей
    // Путь к каталогу с CSV-файлами для загрузки
    #[arg(long)]
    // говорит о том что следующая строка будет аргументом, а long - указывает на длину имени
    ...
    // value_enum для того чтобы clap использовал enum как пул значений аргумента
...
pub fn get_args() -> Result<Arguments, String> {
    ...
    if args.batch_size == 0 || args.batch_size > 1000000 {
        return Err(
            "Ошибка: Количество строк в буфере должно быть > 0 и ≤ 1 000 000 
        (batch-size должен быть от 1 до 1 000 000.)"
                .to_string(),
        ); // eprintln!() - для того чтоббы вывело в поток ошибок
    }
    // Проверка каталога
    ...
    if !args.db_connection.contains("host=")
        || !args.db_connection.contains("dbname=")
        || !args.db_connection.contains("user=")
    {
